...
async function startServer() {
  ...
  // In-memory registry for automated VPS agent discovery & NAT bypass connections
  ...
  const agentSessions = new Map<string, AgentRegistration>();

  // Cleanup sessions older than 2 hours
  setInterval(() => {
    const twoHoursAgo = Date.now() - 2 * 60 * 60 * 1000;
    for (const [token, session] of agentSessions.entries()) {
      if (session.registeredAt < twoHoursAgo) {
        agentSessions.delete(token);
      }
    }
  }, 15 * 60 * 1000);

  // Helper function to check if IPv4 is RFC1918 Private
  ...
  // Helper function to check if IPv4 is CGNAT (100.64.0.0/10)
  ...
  // Helper to execute command over SSH
  function executeSshCommand(
    ...
    return new Promise((resolve, reject) => {
      ...
      timer = setTimeout(() => {
        ...
        reject(new Error(`Koneksi SSH timeout setelah ${Math.round(timeoutMs / 1000)} detik.`));
      }, timeoutMs);
  ...
  // 1. Check TCP Port Open/Reachable
  app.post('/api/vps/check-port', async (req: Request, res: Response) => {
    const { host, port, timeout = 3000 } = req.body;
    if (!host || !port) {
      return res.status(400).json({ error: 'host dan port diperlukan' });
    }
    ...
    socket.on('connect', () => {
      ...
      socket.destroy();
      return res.json({ open: true, latencyMs, host, port });
    });

    socket.on('timeout', () => {
      socket.destroy();
      return res.json({ open: false, error: 'Connection timed out', host, port });
    });

    socket.on('error', (err) => {
      socket.destroy();
      return res.json({ open: false, error: err.message, host, port });
    });

    socket.connect(parseInt(port, 10), host);
  });

  // 1.1 Automated Network & NAT Classification Engine
  app.post('/api/vps/detect-network', async (req: Request, res: Response) => {
    const { host, port = 22 } = req.body;
    if (!host) {
      return res.status(400).json({ error: 'Host atau IP wajib disertakan' });
    }
    ...
    let classification: 'public' | 'private_rfc1918' | 'cgnat' | 'loopback' | 'domain' = 'public';
    let isNat = false;
    let natReason = '';
    ...
    // Generate unique session token for this resolution
    const sessionToken = `vla_${Date.now().toString(36)}_${Math.random().toString(36).substring(2, 7)}`;
    const appUrl = process.env.APP_URL || `${req.protocol}://${req.get('host')}`;
    const agentCmd = `curl -fsSL "${appUrl}/api/vps/nat-agent?token=${sessionToken}&port=${parsedPort}" | sudo bash`;
    ...
    if (!isNat) {
      ...
      reachError = `Alamat ${cleanHost} berada di subnet privat/NAT dan tidak dapat diakses langsung via internet publik.`;
    }
  ...
  // 1.2 One-Line Automated VPS Discovery & NAT Bypass Script
  app.get('/api/vps/nat-agent', (req: Request, res: Response) => {
    const token = (req.query.token as string) || `vla_${Date.now().toString(36)}`;
    const port = (req.query.port as string) || '22';
    const appUrl = process.env.APP_URL || `${req.protocol}://${req.get('host')}`;
  ...
  // 1.3 Register node from Agent payload
  app.post('/api/vps/agent-register', (req: Request, res: Response) => {
    const {
      token,
      hostname,
      internalIp,
    ...
    if (!token) {
      return res.status(400).json({ success: false, error: 'Token sesi diperlukan' });
    }
  ...
  // 1.4 Check Agent Registration Status (for frontend polling)
  app.get('/api/vps/agent-status/:token', (req: Request, res: Response) => {
    const token = req.params.token;
    if (!token) {
      return res.status(400).json({ registered: false, error: 'Token diperlukan' });
    }
  ...
  // 2. Real SSH Connection Handshake & Probe
  app.post('/api/vps/test-ssh', async (req: Request, res: Response) => {
    const { host, port = 22, username = 'root', password, privateKey } = req.body;

    if (!host) {
      return res.status(400).json({ success: false, error: 'Alamat IP / Host VPS wajib diisi' });
    }

    if (!password && !privateKey) {
      return res.status(400).json({ success: false, error: 'Password atau SSH Private Key diperlukan' });
    }
    ...
    try {
      // Gather system telemetry in 1 quick bash command
      const probeScript = `
      ...
      const result = await executeSshCommand(sshConfig, probeScript, 10000);
      ...
      // Parse kernel
      const unameSection = (output.split('---OS_END---')[1] || '').split('---UNAME_END---')[0].trim();
      const kernelLines = unameSection.split('\n');
      const kernel = kernelLines[0] || 'Linux Kernel';
      const arch = kernelLines[1] || 'x86_64';

      // Parse RAM
      const freeSection = (output.split('---UNAME_END---')[1] || '').split('---FREE_END---')[0];
      ...
      const availableRamMb = parseInt(memParts[6], 10) || (totalRamMb - usedRamMb);
      const actualUsedRamMb = totalRamMb - availableRamMb;
      const ramPct = Number(((actualUsedRamMb / totalRamMb) * 100).toFixed(1));

      // Parse Disk
      const dfSection = (output.split('---FREE_END---')[1] || '').split('---DF_END---')[0];
      const dfLine = dfSection.split('\n').find((l) => l.includes('/')) || '';
      const dfParts = dfLine.trim().split(/\s+/);
      const diskTotalMb = parseInt(dfParts[1], 10) || 50000;
      const diskUsedMb = parseInt(dfParts[2], 10) || 5000;
  ...
  // 3. Real Live System Metrics & Accurate Panel/OS Telemetry
  app.post('/api/vps/real-metrics', async (req: Request, res: Response) => {
    const { host, port = 22, username = 'root', password, privateKey } = req.body;

    if (!host || (!password && !privateKey)) {
      return res.status(400).json({ error: 'Kredensial SSH tidak lengkap' });
    }
    ...
    try {
      ...
      const result = await executeSshCommand(sshConfig, script, 10000);
      ...
      // 2. Parse Kernel & Arch
      const unameSection = (out.split('===OS_END===')[1] || '').split('===UNAME_END===')[0].trim();
      const kernelLines = unameSection.split('\n');
      const kernel = kernelLines[0] || 'Linux Kernel';
      const arch = kernelLines[1] || 'x86_64';

      // 3. RAM
      const freePart = (out.split('===UNAME_END===')[1] || '').split('===DF===')[0];
      const memLine = freePart.split('\n').find((l) => l.trim().startsWith('Mem:')) || '';
      const memTokens = memLine.trim().split(/\s+/);
      const totalMb = parseInt(memTokens[1], 10) || 4096;
      const usedMb = parseInt(memTokens[2], 10) || 350;
      const availMb = parseInt(memTokens[6], 10) || (totalMb - usedMb);
      const realUsedMb = totalMb - availMb;
      const ramPct = Number(((realUsedMb / totalMb) * 100).toFixed(1));

      // 4. Disk
      const dfPart = (out.split('===DF===')[1] || '').split('===LOAD===')[0];
      const dfLine = dfPart.split('\n').find((l) => l.includes('/')) || '';
      const dfTokens = dfLine.trim().split(/\s+/);
      const diskTotal = parseInt(dfTokens[1], 10) || 50000;
      const diskUsed = parseInt(dfTokens[2], 10) || 5000;
      const diskPct = Number(((diskUsed / diskTotal) * 100).toFixed(1));

      // 5. Load avg
      const loadPart = (out.split('===LOAD===')[1] || '').split('===UPTIME===')[0].trim();
      const loadTokens = loadPart.split(/\s+/);
      const loadAvg = [
        parseFloat(loadTokens[0]) || 0.1,
        parseFloat(loadTokens[1]) || 0.1,
        parseFloat(loadTokens[2]) || 0.1,
      ];
      ...
      // 7. Ports & Services to detect real active panel
  ...
  // 4. Real Shell Command Execution (Live Web Terminal Bridge)
  app.post('/api/vps/exec', async (req: Request, res: Response) => {
    const { host, port = 22, username = 'root', password, privateKey, command, timeout = 900000 } = req.body;
    ...
    if (!command) {
      return res.status(400).json({ error: 'Perintah (command) wajib disertakan' });
    }
    ...
    try {
      // Timeout hingga 15 menit untuk instalasi panel panjang
      const result = await executeSshCommand(sshConfig, command, Math.max(timeout, 900000));
      return res.json({
        success: true,
